...
#combine native am/pacific islander groups
...
#combine insured/no specifics and insured
...
colnames(cancer_data) <- c("X1", "Study_ID", "Sex", "Year_of_Diagnosis", "Age_of_Diagnosis", "Race", "Insurance", "SEER_Registry", "%_<_9th_grade_ed", "%_<High_school_ed", "%_<Bachelors_ed", "%_Persons_below_poverty","%_Unemployed_2013-2017", "Median_Household_Income", "%_Language_Isolation", "Site", "Subsite", "AJCC_7_Stage", "Size", "Lymph_nodes", "Mets", "Cause_of Death","Surgery_Performed","Surgery_Decision", "Radiation", "Chemotherapy" )
...
ggplot(data=vis_frame, aes(x=size, group=race, fill=race)) +
...
black_white_vis <- vis_frame%>%filter(race%in%c("Black", "White"))
ggplot(data=black_white_vis, aes(x=size, group=race, fill=race)) +
    geom_density(adjust=1.5, alpha=.4) +
    theme_ipsum() + ggtitle("Tumor Size")
...
den_oral <- vis_frame%>%filter(race%in%c("White", "Black"))%>%filter(site == "Oral Cavity")
...
cancer_data%>%filter(Site== "Oral Cavity")%>%
ggplot(data =seer_data, mapping = aes(x=Insurance, fill=Surgery_Performed), position="fill") +
  geom_bar(alpha=1/2, poistion="identity" )  + ggtitle("Insurance of Patients Who had Surgery - by Race")+
    theme(legend.position = 'top')+
  facet_wrap(~Race)+
  scale_fill_brewer(palette="Set1")+
...
insurance_df<- cancer_data%>%filter(Site== "Oral Cavity")


ggplot(data =insurance_df, mapping = aes(x=Insurance, fill = Race)) +
  geom_bar(alpha=1/2, poistion="identity" ) +
  facet_wrap(~Surgery_Performed)+ theme(legend.position = 'top')+
  scale_fill_brewer(palette="Spectral")+
  theme(legend.title = element_text(size = 5), 
                       legend.text = element_text(size = 5))+
  theme_minimal() + theme(panel.background = element_rect(colour = "lightblue", size=2))+ggtitle("Oral Cavity: Surgery Performed")
...
- For Report - Surgery Decision by State
```{r}
cancer_data_oc <- cancer_data%>%filter(Site == "Oral Cavity")%>%
  filter(Surgery_Performed == "Yes")%>%select(Race,Site,SEER_Registry, Surgery_Performed, AJCC_7_Stage)

ggplot(data = cancer_data_oc)+
  geom_bar(mapping = aes(x = AJCC_7_Stage, fill=Race), position="fill")+
  facet_wrap(~SEER_Registry)
...
```{r}
seer_black_white <- cancer_data%>%filter(Race%in%c("Black","White"))

ggplot(data = seer_black_white)+
  geom_bar(mapping = aes(x = AJCC_7_Stage, fill=Surgery_Performed), position="fill")+ facet_wrap(~Race)+theme_minimal()+
  scale_fill_brewer(palette="Spectral")+ theme(panel.background = element_rect(colour = "darkgreen", size=2))+ 
   ggtitle("Black and White Races: Surgery Performed by Stage")
...
df_g<- black_white%>%filter(SEER_Registry== "Georgia")%>%
  filter(AJCC_Stage%in%c("I", "II", "III", "IV", "IVA", "IVB", "IVC"))

ggplot(data = df_g)+
  geom_bar(mapping =  aes(x = AJCC_Stage, fill=Surgery_Performed), position="dodge")+
  facet_wrap(~Race)+ theme(legend.position = 'top')+
...
```{r}
ggplot(data = df_g)+
  geom_bar(mapping =  aes(x = AJCC_Stage, fill=Surgery_Performed), position="fill")+
  facet_wrap(~Race)+ theme(legend.position = 'top')+
...
ggplot(data = cancer_data)+
  geom_bar(mapping =  aes(x = AJCC_7_Stage, fill= Surgery_Performed), position="fill")+
  facet_wrap(~Race)+ theme(legend.position = 'top')+
  scale_fill_manual(values=c("#999999", "#f2f40b", "#56B4E9"))+ theme_minimal()+
  ggtitle("White and Black Races in Georgia - All Sites")
...
df_oc2<- cancer_data%>%filter(Site == "Oral Cavity")%>%filter(AJCC_7_Stage%in%c("I", "II", "III", "IV", "IVA", "IVB", "IVC"))
      ...
      geom_bar(mapping =  aes(x = AJCC_7_Stage,  fill=Surgery_Performed), position="fill")+
        facet_wrap(~Race)+ 
          scale_fill_brewer(palette="Identity")+
              theme_minimal() + 
                theme(panel.background = element_rect(colour = "lightblue", size=2))+ theme(legend.title = element_text(size = 6), 
                       legend.text = element_text(size = 6))+
                         ggtitle("Oral Cavity: All Races")
...
df_oc2_georgia<- df_oc2%>%filter(SEER_Registry== "Georgia")



ggplot(data = df_oc2_georgia)+
  geom_bar(mapping =  aes(x = AJCC_7_Stage,  fill=Surgery_Performed), position="fill")+
  facet_wrap(~Race)+ theme_minimal()+
  scale_fill_manual(values=c("#96e59e", "#595ea4"))+
  theme(panel.background = element_rect(colour = "lightblue", size=2))+
                   theme(legend.title = element_text(size = 8), 
                       legend.text = element_text(size = 8))+
   ggtitle("Georgia: Oral Cavity")
...
- for report
```{r}
df_oc2_cali <- cancer_data%>%filter(SEER_Registry == "California")


ggplot(data = df_oc2_cali)+
  geom_bar(mapping =  aes(x = AJCC_7_Stage,  fill=Surgery_Performed), position="fill")+
  facet_wrap(~Race)+ theme_minimal()+
  scale_fill_manual(values=c("#96e59e",  "#9999CC","#595ea4"))+
    theme(panel.background = element_rect(colour = "lightblue", size=2))+
                   theme(legend.title = element_text(size = 7), 
                       legend.text = element_text(size = 7))+
   ggtitle("California- Oral Cavity")
...
```{r}
df_oc2_conn <- cancer_data%>%filter(SEER_Registry == "Connecticut")


ggplot(data = df_oc2_conn)+ theme_minimal()+
  geom_bar(mapping =  aes(x = AJCC_7_Stage,  fill=Surgery_Performed), position="fill")+
  facet_wrap(~Race)+ 
  scale_fill_manual(values=c("#96e59e",  "#CC6666","#595ea4"))+
    theme(panel.background = element_rect(colour = "lightblue", size=2))+
  theme(legend.title = element_text(size = 8), 
                       legend.text = element_text(size = 8))+
   ggtitle("Connecticut - Oral Cavity ")
